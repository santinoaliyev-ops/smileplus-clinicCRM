...
import { useDoctorInvoices } from "@/features/doctor-dashboard/hooks/useDoctorInvoices";
import { DoctorDeskLayout } from "@/pages/doctor-desk/DoctorDeskLayout";
import { DoctorInvoiceDetailPanel } from "./DoctorInvoiceDetailPanel";

type Filter = "unpaid" | "partial" | "paid" | "all";

export function DoctorInvoicesPage() {
  ...
  const { data: invoices = [], isLoading } = useDoctorInvoices(clinic?.clinicId, doctor?.id);
      ...
      <div className="flex h-full gap-3 overflow-hidden">
        <div className="flex flex-1 flex-col overflow-hidden rounded-2xl bg-white shadow-sm">
          <div className="flex shrink-0 items-center justify-between gap-1 border-b border-gray-100 p-2">
            <h1 className="px-2 text-sm font-bold text-gray-800">{t("doctorInvoices.title")}</h1>
          ...
          <div className="grid shrink-0 grid-cols-[1fr_110px_110px_110px] border-b border-gray-200 bg-gray-50 px-4 py-2 text-[11px] font-semibold uppercase tracking-wide text-gray-500">
            ...
            {isLoading ? (
              ...
              filtered.map((inv) => (
                <button
                  ...
                  className={`grid w-full grid-cols-[1fr_110px_110px_110px] items-center border-b border-gray-50 px-4 py-2.5 text-left transition hover:bg-teal-50 ${
